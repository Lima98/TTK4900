...
At the same time, the objects are still quite lightweight. A \texttt{Note} stores a MIDI number and a duration, but there is still no explicit harmonic plan, no voice profile, and no event metadata for things such as rests or chromatic adjustments. This means that the representation is better than in the first iteration, but still too limited to support the kinds of transformations and constraints that are needed in the final system.
...
\begin{example}
  ...
  \end{center}
  \caption[Demonstration of a melody generated using the second iteration.]{Demonstration of a melody generated using the second iteration. \examplelink.}\label{fig:iter2_melody}
\end{example}
...
\begin{example}
  ...
  \end{center}
  \caption[A second melody from the second iteration, showing the same motif-reuse strategy with a different random outcome.]{A second generated melody from the second iteration, showing the same motif-reuse strategy with a different random outcome. \examplelink.}\label{fig:iter2_melody_alt1}
\end{example}
...
\begin{example}
  ...
  \end{center}
  \caption[A third melody from the second iteration, again reusing an initial motif across the phrase.]{A third generated melody from the second iteration, again reusing an initial motif across the phrase. \examplelink.}\label{fig:iter2_melody_alt2}
\end{example}
...
\begin{example}
  ...
  \end{center}
  \caption[A fourth melody from the second iteration, illustrating how the same method can still lead to different melodic surfaces.]{A fourth generated melody from the second iteration, illustrating how the same method can still lead to noticeably different melodic surfaces. \examplelink.}\label{fig:iter2_melody_alt3}
\end{example}
...
\section{Discussion}
  ...
  \section{Goals for the next iteration}
  \noindent
